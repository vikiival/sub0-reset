...
mod erc721 {
    ...
    impl Erc721 {
        ...
        #[ink(message)]
        pub fn get_contract_uri(&self) -> Option<String> {
        ...
        #[ink(message)]
        pub fn token_uri(&self, id: TokenId) -> Option<(String, TokenId)> {
            // Task is to take base + / + tokenId and return the uri
            // Base can be empty
            // let token_uri = self.base_uri.clone() + "/" + &id.to_string();

            Some((self.base_uri.clone(), id))
        }

        #[ink(message)]
        pub fn set_base_uri(&mut self, uri: Option<String>) -> Result<(), Error> {
            self.base_uri = uri.unwrap_or_default();
            Ok(())
        }
